  ...
  useGSAP(() => {
    ...
    textElement.innerHTML =
      text
        ...
        .map((char) =>
          char === " "
            ? `<span style="display: inline-block; width: 0.3em; opacity: 0;">&nbsp;</span>`
            : `<span style="display: inline-block; opacity: 0;">${char}</span>`
        )
        .join("") +
      `<span style="display: flex; width:100%; justify-content :center; margin-top:10px; opacity: 0;"> <br/> <img style="width:34px; height:34px;" src=${imojiIcon} alt='' /></span>`;
    ...
    >
      ...
      >
          ...
          >
            {testimonials.map((testimonial, index) => {
              ...
              return (
                <SwiperSlide
                  key={testimonial.id}
                  className={`duration-200 transition-all overflow-hidden pt-2 ease-out flex items-center justify-center ${
                    index === activeIndex
                      ? "custome-scaled"
                      : "custome-without-scale"
                  }`}
        ...
        <div className='card-with-shape content mx-auto col-span-full relative items-center gap-3 bg-white flex flex-col testimonial-shadow py-6'>
          ...
          <div className='absolute left-9 top-2 md:left-14'>
            <img
              src={quoteIcon}
          ...
          <p
            ref={textRef}
            className='max-w-[762px] px-16 text-center text-black text-2xl font-normal font-inter'
          >
            {activeTestimonial.message}
          </p>
        </div>
...
const Card = ({ testimonial, index, activeIndex }) => {
  ...
  return (
    <div className='card-container' ref={cardRef}>
      <div className='flex flex-col md:flex-row items-center md:items-start p-[5vw] md:p-3 gap-4 justify-center'>
        ...
        <div className='flex flex-col justify-center gap-2'>
          <p className='text-[#232323] font-semibold text-xl' ref={nameRef}>
            {testimonial.name}
          </p>
          <p className='text-[#747474]' ref={companyRef}>
            {testimonial.company}
          </p>
        </div>
      </div>
